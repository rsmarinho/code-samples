...
int mmagicdot(t_matrix * ans, t_matrix * v, t_matrix * k)
{
	int op_type;
	
	for (int i=0; i<v->dim;i++) {
		if (v->n_dim[i] != k->n_dim[i]) {
			printf("mmagicdot: wrong matrix size\n");
			exit(1);
		}
	}
	...
	if (!v->type && !k->type) {
		...
		printf("mmagicdot: wrong matrix type\n");
		exit(1);
	}		
	...
	unsigned int v_size	  = (sizeof(double) + sizeof(double)*v->type);
	unsigned int k_size   = (sizeof(double) + sizeof(double)*k->type);
	unsigned int ans_size   = (sizeof(double) + sizeof(double)*ans->type);
	void *v_val, *k_val;
